  ...
  return (
    <nav className="flex items-center justify-center gap-2" aria-label={copy.nav_aria}>
      ...
      >
        ...
        <span className="ml-1 hidden text-sm font-bold uppercase tracking-wide md:inline">
          {copy.previous}
        </span>
      </Link>
        ...
        {getPageNumbers().map((page, index) => {
          if (page === "...") {
            return (
              <span
                key={`ellipsis-${index}`}
                className="px-2 text-gray-400 md:px-3"
              >
                ...
              </span>
            );
      ...
      >
        <span className="mr-1 hidden text-sm font-bold uppercase tracking-wide md:inline">
          {copy.next}
        </span>
